...
func (l *GormLogger) Info(ctx context.Context, msg string, data ...interface{}) {
	CtxInfo(ctx, "GORM LOG %s %+v", msg, data)
}

func (l *GormLogger) Warn(ctx context.Context, msg string, data ...interface{}) {
	CtxWarn(ctx, "GORM LOG %s %+v", msg, data)
}

func (l *GormLogger) Error(ctx context.Context, msg string, data ...interface{}) {
	CtxError(ctx, "GORM LOG %s %+v", msg, data)
}
...
func (l *GormLogger) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
	if l.LogLevel > logger.Silent {
		...
		case err != nil && l.LogLevel >= logger.Error && !errors.Is(err, gorm.ErrRecordNotFound) && !isEntryDuplicateErr(err):
			sql, _ := fc()
			CtxError(ctx, "GORM LOG: %s, Err: %s, Cost: %.2fms", sql, err.Error(), cost)
		...
		case l.LogLevel >= logger.Warn && costDuration > l.SlowThreshold && l.SlowThreshold > 0:
			sql, rows := fc()
			CtxWarn(ctx, "GORM LOG SLOW SQL: %s, Rows: %d, Cost: %.2fms, Limit: %s", sql, rows, cost, l.SlowThreshold)

		// normal SQL record
		case l.LogLevel >= logger.Info:
			sql, rows := fc()
			CtxInfo(ctx, "GORM LOG SQL: %s, Rows: %d, Cost: %.2fms", sql, rows, cost)
		}
	}
}
